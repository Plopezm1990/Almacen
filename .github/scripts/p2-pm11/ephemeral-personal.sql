...
-- Reaplicación controlada sobre estado con datos: simula QA ya adelantado y
-- demuestra que la reconciliación no altera filas existentes ni degrada PM13.
create temp table p2_pm11_snapshot as
...
select public.p2_pm11_assert(
  not exists(
    (select * from p2_pm11_snapshot except select id,empresa_id,local_id,estado,nombre,datos,created_at,updated_at,baja_at,reactivado_at,anonimizado_at from public.empleados)
    union all
    (select id,empresa_id,local_id,estado,nombre,datos,created_at,updated_at,baja_at,reactivado_at,anonimizado_at from public.empleados except select * from p2_pm11_snapshot)
  ),
  'reapply changed existing employees'
);

select public.p2_pm11_assert(
  position('pm13:empleado:alta:' in pg_get_functiondef('public.pm11_alta_empleado(text,text,text,text,jsonb)'::regprocedure)) > 0,
  'pm13 alta advisory lock regressed'
);
select public.p2_pm11_assert(
  position('personal_local_concreto_requerido' in pg_get_functiondef('public.pm11_alta_empleado(text,text,text,text,jsonb)'::regprocedure)) > 0,
  'pm13 concrete local guard regressed'
);
select public.p2_pm11_assert(
  position('almacen_kv' in lower(pg_get_functiondef('private.pm11_local_pertenece_empresa(text,text)'::regprocedure))) = 0
  and position('from public.locales' in lower(pg_get_functiondef('private.pm11_local_pertenece_empresa(text,text)'::regprocedure))) > 0
  and position('almacen_kv' in lower(pg_get_functiondef('private.pm11_local_activo(text,text)'::regprocedure))) = 0
  and position('from public.locales' in lower(pg_get_functiondef('private.pm11_local_activo(text,text)'::regprocedure))) > 0,
  'relational local authority not restored'
);
select public.p2_pm11_assert(
  (select array_to_string(coalesce(proconfig,array[]::text[]),',') like '%search_path=""%'
     from pg_proc where oid='private.pm11_puede_mutar_personal(text,text)'::regprocedure)
  and
  (select array_to_string(coalesce(proconfig,array[]::text[]),',') like '%search_path=""%'
     from pg_proc where oid='private.pm11_puede_migrar_personal(text,text)'::regprocedure),
  'private helper search_path not hardened'
);

select 'P2_PM11_QA_RECONCILIATION_OK=1' as result;
