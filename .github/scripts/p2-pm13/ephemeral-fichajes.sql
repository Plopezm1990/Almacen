...
-- Compatibilidad QA-like: filas históricas preexistentes, sin operationId.
-- Deben sobrevivir byte-semánticamente a ambas migraciones PM13.
insert into public.fichajes_registro(id,fecha,datos)
values
 ('legacy-pre-1',current_date-10,jsonb_build_object(
    'id','legacy-pre-1','empleadoId','emp-a','localId','l1',
    'fecha',(current_date-10)::text,'hora','07:00','tipo','entrada',
    'timestamp',(current_date-10)::text||'T07:00:00','manual',false,'anulado',false
 )),
 ('legacy-pre-2',current_date-10,jsonb_build_object(
    'id','legacy-pre-2','empleadoId','emp-a','localId','l1',
    'fecha',(current_date-10)::text,'hora','15:00','tipo','salida',
    'timestamp',(current_date-10)::text||'T15:00:00','manual',false,'anulado',false
 ));

create temp table p2_pm13_preexisting_snapshot as
select id,fecha,datos,creado_en
from public.fichajes_registro
where id in ('legacy-pre-1','legacy-pre-2')
order by id;
...
select public.p2_pm13_assert(
  not exists(
    (select * from p2_pm13_preexisting_snapshot
      except
     select id,fecha,datos,creado_en from public.fichajes_registro where id in ('legacy-pre-1','legacy-pre-2'))
    union all
    (select id,fecha,datos,creado_en from public.fichajes_registro where id in ('legacy-pre-1','legacy-pre-2')
      except
     select * from p2_pm13_preexisting_snapshot)
  ),
  'PM13 changed preexisting fichajes'
);
select public.p2_pm13_assert(
  (select count(*)=2 from public.fichajes_registro where id in ('legacy-pre-1','legacy-pre-2')),
  'PM13 lost preexisting fichajes'
);
select 'P2_PM13_PREEXISTING_ROWS_OK=1' as result;
