// Prueba temporal y controlada de push real, solo en la rama de seguridad.
// Obtiene únicamente la suscripción push de ESTE navegador y la envía a una
// Edge Function temporal que no consulta la tabla suscripciones_push.
(function () {
  "use strict";

  var MARCA = "chocoloyos:push-prueba-real-enviada-v1";
  var URL = "https://flqercbgpgmmfaakrwkc.supabase.co/functions/v1/enviar-notificacion-dispositivo-prueba";
  ...
  function crearPanel() {
    ...
    panel.style.cssText = "position:fixed;left:12px;right:12px;top:12px;z-index:10050;background:#F7F3E9;color:#17241C;border:2px solid #8C6D2A;border-radius:16px;padding:12px 14px;font-family:system-ui,sans-serif;box-shadow:0 6px 24px rgba(0,0,0,.22)";
    ...
    if (localStorage.getItem(MARCA) === "1") {
      ...
      estado.textContent = "Este dispositivo ya completó la prueba temporal.";
    }

    boton.onclick = async function () {
      boton.disabled = true;
      boton.style.opacity = "0.6";
      estado.textContent = "Comprobando la suscripción de este dispositivo…";
      ...
      try {
        ...
        if (!sub) {
          throw new Error("Este dispositivo no tiene una suscripción push activa. No se ha enviado nada.");
        }
        ...
        estado.textContent = "✅ Envío realizado únicamente a la suscripción de este dispositivo. Espera a recibir el aviso.";
      } catch (e) {
  ...
  function iniciar() {
    // Espera a que la app haya inicializado getSupabaseClient y el service worker.
    var intentos = 0;
