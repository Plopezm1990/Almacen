...
Rama: `claude/punto2-134-pruebas`, commit **`c685907ea7a0ba5eb4756e9393491ff4baa3ee1d`**, creada desde `release` (`6e26391eff7bafc1ceb3f1e6f6e45d84f3d3086d`). Fuente de verdad de la clasificación: `cierre-proyecto-a/punto2/manifiesto_clasificacion.json`. Ver `INFORME_CLASIFICACION.md` para la metodología completa, la corrección tras la auditoría independiente, y por qué esta matriz no reproduce el informe original perdido.

## Puerta de CI -- resultado real, no simulado

[`run 35495162728`](https://github.com/Plopezm1990/Almacen/actions/runs/35495162728) -- **SUCCESS**, los 4 jobs (`node-y-postgres`, `pm12-p08-supabase-full`, `pm33-p05-supabase-full`, `gate-final`) en verde. Salida real del job `gate-final`:
...
- **Evidencia**: CI run 35495162728, job node-y-postgres, contra PostgreSQL 16.15 real (servicio de GitHub Actions).
...
- **Evidencia**: Utilidad de aplicación (integra pm12-p09-historial-informes-movil-v1.js en index.html), idempotente. No es un caso de prueba PASS/FAIL. Ejecutada dentro de la batería Node (ver resultado real). CI run 35495162728, job node-y-postgres.
...
- **Evidencia**: Diagnóstico de solo lectura (inspecciona fuente.js, escribe evidencia JSON). Sin PASS/FAIL. Ejecutado dentro de la batería Node (ver resultado real); su evidencia ya commiteada quedó desactualizada respecto al fuente.js actual -- deuda separada, no corregida en esta rama (ver informe). CI run 35495162728, job node-y-postgres.
...
- **Evidencia**: Igual que P01. CI run 35495162728, job node-y-postgres.
...
- **Evidencia**: Registro histórico retirado (P03 cambió el contrato deliberadamente); contrato vigente real: contrato-vigente-contract.mjs (PASS). CI run 35495162728, job node-y-postgres.
