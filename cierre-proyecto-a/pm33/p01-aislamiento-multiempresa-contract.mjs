// PM33 P01: prueba de aislamiento multiempresa real (Postgres, no regex)
// para public.obtener_contexto_operativo().
//
...
//
// No crea ni destruye la base: solo consulta. Documentado como parte del gate
// de PostgreSQL (ver P4, workflow de CI todavía por diseñar).
import pg from 'pg';
...
async function main() {
  ...
  console.log('\n=== T14: Rol legítimo no gestionado por esta RPC sigue funcionando (sin regresión) ===');
  {
    ...
    check('T14c empresaId permanece null (no se le exigió contexto)', ctx.empresaId === null);
    check('T14d proveedores vacío', Array.isArray(ctx.proveedores) && ctx.proveedores.length === 0);
  }

  console.log(`\nPM33_P01_CHECKS=${pass + fail}`);
  console.log(`TOTAL PASS=${pass} FAIL=${fail}`);
  if (fail > 0) console.log('Fallos:', failures.join(', '));
  await client.end();
  if (fail > 0) throw new Error('PM33_P01_AISLAMIENTO_CONTRACT_FAIL');
  console.log('PM33_P01_AISLAMIENTO_CONTRACT_OK=1');
}
