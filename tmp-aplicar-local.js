...
function replaceOnce(oldText, newText, label) {
  ...
  if (first < 0) throw new Error(`No se encontró ancla: ${label}`);
  const second = src.indexOf(oldText, first + oldText.length);
  if (second >= 0) throw new Error(`Ancla duplicada: ${label}`);
  src = src.slice(0, first) + newText + src.slice(first + oldText.length);
  cambios.push(label);
}
function replaceRegexOnce(regex, newText, label) {
  const matches = [...src.matchAll(regex)];
  if (matches.length !== 1) throw new Error(`Regex ${label}: esperada 1 coincidencia, hay ${matches.length}`);
  src = src.replace(regex, newText);
  cambios.push(label);
}
...
function GestionAlmacen() {`,
  'componente selector informes'
);

replaceOnce(
  '  const [localActivoId, setLocalActivoId] = (0, import_react4.useState)(null);\n  const productosDelLocalActivo =',
  '  const [localActivoId, setLocalActivoId] = (0, import_react4.useState)(null);\n  const [localInformeId, setLocalInformeId] = (0, import_react4.useState)("");\n  const productosDelLocalActivo =',
  'estado local de informes'
);

replaceRegexOnce(
  /      let localesFinales = loc \|\| \[\];[\s\S]*?      setProductos\(productosFinales\);/g,
...
replaceOnce(
  ...
  'factura directa nueva con local'
);
replaceOnce(
  '          categoria: f2.categoria || ""\n        });',
  '          categoria: f2.categoria || "",\n          localId: f2.localId || localActivoId || null\n        });',
  'gasto de factura conserva local'
);

replaceOnce(
  '  setPedidoParaFotoIA,\n  setTab\n}) {',
  '  setPedidoParaFotoIA,\n  setTab,\n  localActivoId\n}) {',
  'firma albaranes local'
);
replaceOnce(
...
replaceOnce(
  ...
  'conectar encargos local'
);
replaceOnce(
  '    setPedidoParaFotoIA,\n    setTab\n  });',
  '    setPedidoParaFotoIA,\n    setTab,\n    localActivoId\n  });',
  'conectar albaranes local'
);
replaceOnce(
...
replaceOnce(
  ...
  'conectar facturas directas local'
);

replaceOnce(
  '"Primer paso de varios locales: por ahora solo gestiona la lista y cu\\xE1l est\\xE1 activo en este dispositivo. Todav\\xEDa no reparte productos, stock ni caja por local \\u2014 eso viene despu\\xE9s."',
  '"La separaci\\xF3n por local ya est\\xE1 activa en Panel general, Resultados y Libro de IVA. El resto de m\\xF3dulos mantiene de momento la vista conjunta mientras se completa la separaci\\xF3n por local."',
  'aviso locales actualizado'
);

replaceOnce(
  '  const contenido = /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, tab === "dashboard" &&',
  ...
  const contenido = /* @__PURE__ */ import_react4.default.createElement(import_react4.default.Fragment, null, (tab === "dashboard" || tab === "resultados" || tab === "libroiva") && /* @__PURE__ */ import_react4.default.createElement(SelectorLocalInformes, { locales, valor: localInformeId, onChange: setLocalInformeId }), tab === "dashboard" &&`,
  'datos filtrados y selector solo en tres módulos'
);
...
replaceOnce(
  ...
  'props resultados filtradas'
);

replaceOnce(
  '  ), tab === "libroiva" && /* @__PURE__ */ import_react4.default.createElement(LibroIva, { movimientos, productos, albaranes, proveedorPorId, facturasDirectas }),',
  '  ), tab === "libroiva" && /* @__PURE__ */ import_react4.default.createElement(LibroIva, { movimientos: movimientosInforme, productos: productosInforme, albaranes: albaranesInforme, proveedorPorId, facturasDirectas: facturasDirectasInforme }),',
  'props libro IVA filtradas'
);

if (src === original) throw new Error('No se aplicó ningún cambio');
fs.writeFileSync('fuente.js', src);
fs.mkdirSync('tmp-inspeccion', { recursive: true });
fs.writeFileSync('tmp-inspeccion/patch-check.txt', `CAMBIOS ${cambios.length}\n${cambios.join('\n')}\nSIZE_ANTES ${original.length}\nSIZE_DESPUES ${src.length}\n`);
