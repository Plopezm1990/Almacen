...
-- Ejecutar independientemente contra PROD y QA.
-- No contiene DDL, DML ni cambios de configuración.
...
tables AS (
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'owner', r.rolname
  ) ORDER BY n.nspname, c.relname) AS data
...
policies AS (
  ...
  ) ORDER BY n.nspname, c.relname, pol.polname) AS data
...
triggers AS (
  ...
  ) ORDER BY n.nspname, c.relname, t.tgname) AS data
