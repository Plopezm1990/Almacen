...
check('POSTFLIGHT_C13_HELPER_DOS_ESTADOS_FAIL_CLOSED',
  ...
  && postflight.includes('cross join lateral unnest(coalesce(p.proconfig, array[]::text[])) cfg')
  && postflight.includes("cfg = 'search_path=\"\"'")
  && !postflight.includes("like '%search_path=_%'")
  && postflight.includes('pg_catalog.aclexplode(')
  && postflight.includes('acl.grantee = 0')
  && postflight.includes("acl.privilege_type = 'EXECUTE'")
  && !postflight.includes("has_function_privilege('public'")
