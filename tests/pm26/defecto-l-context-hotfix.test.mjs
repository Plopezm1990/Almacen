...
// 1) Contexto inequívoco: reconstruye empresa/local y completa el POST.
{
  ...
  assert.equal(env.appended.length, 1, 'debe conservar el loader PM11 existente');

  await env.window.fetch('https://example.supabase.co/rest/v1/prefiltros_candidatos', {
    method: 'POST',
    body: json({ token: 'abc', candidato_nombre: 'PRUEBA PM26', estado: 'pendiente' }),
  });
...
// 5) Deploy Preview conserva el comportamiento QA existente.
{
...
// 6) En el repo real el script de compatibilidad debe cargar antes del bundle.
...
if (fs.existsSync(indexPath)) {
  ...
  assert.ok(resetPos < fuentePos, 'el hotfix debe instalarse antes de fuente.js');
}

console.log('PM26_DEFECTO_L_CONTEXT_HOTFIX=PASS');