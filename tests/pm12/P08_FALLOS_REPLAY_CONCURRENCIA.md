...
## Auditoría histórica remota, solo lectura

El 7 de septiembre de 2026 se auditaron los entornos remotos autorizados. Solo se ejecutaron consultas de metadatos y `SELECT`; no hubo escrituras, DDL, migraciones ni despliegues.

El universo histórico verificable de conteos y ajustes es vacío en los entornos revisados. No existe ningún efecto antiguo que corregir o migrar.

Los documentos con efectos no reconciliados seguirán bloqueándose con `conteo_legacy_requiere_revision` si aparecieran en otro entorno o tras una importación. La reparación manual de duplicados antiguos tampoco aplica stock fuera del backend. Sin conexión se conserva el conteo y se exige confirmación atómica. Contra un backend sin las nuevas funciones, el cliente devuelve error sin aplicar stock. Cualquier despliegue remoto requiere autorización separada. No avanzar a P09.
...
- Garantía probada: lote local prevalidado, replay determinista, transacción PostgreSQL y flujo real Auth→JWT→PostgREST→RLS en una pila Supabase completa y aislada. El contenido histórico remoto autorizado fue auditado y no contiene ajustes de conteo que reconciliar. No se acredita despliegue.
- Esta auditoría y sus pruebas no realizaron escrituras en Supabase remoto ni en producción. Las migraciones históricas, pruebas P02–P07 y diagnósticos P09 no se modificaron. Se ajustó exclusivamente el control de archivos del workflow P07: admitía cero archivos SQL y confundía incluir una migración local con escribir en Supabase remoto. Ahora permite solo la migración P08 identificada; las pruebas P07 y P02–P06 siguen intactas. El primer gate P08 remoto (34142435030, commit 14215df1deab5ff5447316d888e4b106fa0f54df) pasó completo; el control anterior de P07 falló después de pasar todas sus pruebas. El gate final posterior quedó en SUCCESS.
