#!/usr/bin/env bash
# Puerta de CI -- batería reproducible de los contratos que NO requieren
# Postgres/Auth/PostgREST.
#
# Infraestructura general del repositorio (Punto 3), generalizada a partir
# de la batería validada en el Punto 2 (cierre-proyecto-a/punto2/), con la
# que comparte lógica pero no rutas: vive bajo tests/ci/, no bajo
# cierre-proyecto-a/, para poder ejecutarse sobre cualquier rama --
# incluidos los PR dirigidos a `release` -- sin depender de documentación
# de cierre de un punto concreto.
#
...
# (tests/ci/manifiesto_clasificacion.json) y del árbol real de `tests/`,
# verifica el número esperado ANTES de ejecutar nada, y deja el árbol de
# trabajo limpio al terminar (revierte cualquier mutación de los scripts
# de diagnóstico/utilidad, borra el artefacto de build de Netlify y la
# propia evidencia bruta generada, que vive en tests/ci/evidencia/,
...
# bajo tests/ci/evidencia/, ignorado por git y por el propio build de
# Netlify porque tests/ ya está excluido en bloque) no afectarían a esa
# copia, pero se prefiere mantener el mismo orden ya validado en el Punto
# 2 por prudencia.
...
# ---- 7. Verificación de limpieza: el árbol debe quedar limpio de verdad,
# sin ninguna exclusión -- la evidencia bruta vive en tests/ci/evidencia/,
# ignorada por git, así que no hace falta excluir ninguna ruta del propio
# repositorio de cierre para que esta comprobación tenga sentido. Se
# comprueba DESPUÉS de haber calculado e impreso el veredicto de arriba,
# nunca antes -- así un árbol sucio nunca enmascara un fallo real de un
# contrato activo (ni al revés). ----
