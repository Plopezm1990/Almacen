-- PM27 C20 — Inventarios/conteos: identidad de replay y cancelacion canonica.
-- Candidato de codigo solamente. No aplica cambios remotos por si mismo.
...
begin
  if to_regprocedure('private.pm12_confirmar_ajuste_stock(text,text,text,jsonb,jsonb,jsonb)') is null then
    raise exception 'missing_dependency:private.pm12_confirmar_ajuste_stock';
  end if;
  if to_regprocedure('private.pm12_cancelar_conteo_stock(text,text,jsonb,jsonb)') is null then
    raise exception 'missing_dependency:private.pm12_cancelar_conteo_stock';
  end if;
  if to_regprocedure('private.pm09_bloquear_operation_id_stock(text)') is null then
    raise exception 'missing_dependency:private.pm09_bloquear_operation_id_stock';
  end if;
  if to_regprocedure('private.pm08_bloquear_operation_id(text)') is null then
    raise exception 'missing_dependency:private.pm08_bloquear_operation_id';
  end if;
end $$;
...
begin
  ...
  -- Serializa por documento y comparte el namespace global de operaciones de stock.
  ...
  if found then
    -- Un replay solo es valido para la misma intencion y el mismo plan. Desde C20
    -- las bases tambien quedan persistidas y deben coincidir. Operaciones PM12
    -- historicas sin `bases` mantienen compatibilidad, pero nunca pueden cambiar plan.
    if op.tipo <> 'INVENTARIO_PM12' or op.empresa_id <> p_empresa_id or op.local_id <> p_local_id
       or op.payload->'intencion' is distinct from intent
       or op.payload->'plan' is distinct from p_plan
       or (op.payload ? 'bases' and op.payload->'bases' is distinct from p_bases)
  ...
  -- Lock estable antes de validar stock base y antes de cualquier escritura.
  ...
  insert into public.stock_operaciones(operation_id,tipo,empresa_id,local_id,producto_id,payload,actor_user_id)
    values(
      p_operation_id,'INVENTARIO_PM12',p_empresa_id,p_local_id,'__CONTEO__',
      jsonb_build_object('intencion',intent,'plan',p_plan,'bases',p_bases),auth.uid()
    );
...
declare
  ...
  original_id text; cancel_id text; corte text; corte_solicitado text; reverse_id text; result jsonb; document jsonb;
...
begin
  ...
  -- El documento es la frontera de concurrencia. Primero se serializa por conteo.
  ...
  select * into original
   ...
   where tipo='INVENTARIO_PM12'
     ...
     and payload->'intencion'->>'id'=p_conteo->>'id'
   for update;

  corte_solicitado := coalesce(nullif(p_conteo->>'cerradoEn',''),nullif(p_conteo->>'iniciadoEn',''),nullif(p_conteo->>'fecha',''),'sin-corte');
  if found then
    -- Si ya existe el ledger del conteo, su corte persistido es la identidad canonica.
    corte := coalesce(nullif(original.payload->'intencion'->>'cerradoEn',''),nullif(original.payload->'intencion'->>'iniciadoEn',''),nullif(original.payload->'intencion'->>'fecha',''),'sin-corte');
    if corte_solicitado is distinct from corte then raise exception 'operation_id_conflict'; end if;
  else
    corte := corte_solicitado;
  end if;
  ...
  if found then
    ...
    -- El mismo operationId no puede reinterpretarse con otro motivo/responsable.
    if coalesce(previous.payload->'cancelacion'->>'motivo',previous.payload->'resultado'->'conteo'->>'motivoCancelacion') is distinct from p_cancelacion->>'motivo'
       or coalesce(previous.payload->'cancelacion'->>'responsable',previous.payload->'resultado'->'conteo'->>'responsableCancelacion') is distinct from p_cancelacion->>'responsable'
    then raise exception 'operation_id_conflict'; end if;
  ...
  if not found and original.operation_id is null then
    ...
    insert into public.stock_operaciones(operation_id,tipo,empresa_id,local_id,producto_id,payload,actor_user_id)
      values(original_id,'INVENTARIO_PM12',p_empresa_id,p_local_id,'__CONTEO__',jsonb_build_object('intencion',p_conteo,'plan','[]'::jsonb,'bases','[]'::jsonb),auth.uid()) returning * into original;
  end if;
  ...
  insert into public.stock_operaciones(operation_id,tipo,empresa_id,local_id,producto_id,payload,actor_user_id,ref_operation_id)
    values(
      cancel_id,'REVERSO',p_empresa_id,p_local_id,'__CONTEO__',
      jsonb_build_object('conteoId',p_conteo->>'id','cancelacion',p_cancelacion),
      auth.uid(),original.operation_id
    );
